...
description: >-
  UnoCSS 变换器，用于 `@apply`、`@screen` 和 `theme()`
  指令（@unocss/transformer-directives）
---

# 指令变换器

UnoCSS 变换器，用于 `@apply`、`@screen` 和 `theme()` 指令：`@unocss/transformer-directives`。
...
为了兼容原生 CSS，您可以使用 CSS 自定义属性来替代 `@apply` 指令：
...
此功能默认启用，具有一些别名，您可以通过以下方式进行配置或禁用：
...
在 `@apply` 后使用引号是可选的，以满足某些格式化器的行为。

### `@screen`

`@screen` 指令允许您通过名称引用您的断点来创建媒体查询，来源于 [`theme.breakpoints`](/config/theme)。
...
::: warning
`icon()` 依赖于 `@unocss/preset-icons` 并将使用该配置，请确保您已添加此预设。
:::
...
由于图标默认使用 `currentColor` 作为填充颜色，如果您想自定义图标的颜色，可以使用 `icon('图标名称', '自定义颜色')`。
...
* MIT 许可证 © 2022-PRESENT [hannoeru](https://github.com/hannoeru)
